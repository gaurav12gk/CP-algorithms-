...
template <class T, class V> void _print(map <T, V> v) {cerr << "[ "; for (auto i : v) {_print(i); cerr << " ";} cerr << "]";}
 ...
 bool isvowel(char v) { return (0x208222>>(v&0x1f))&1; }
    ...
    long long low =0; 
    long long ans = 0 ; 
//calculate the highest value of the vector so that you can set your blade max to that level 
        high = *max_element(v.begin(),v.end()); 
// Now apply binary search on the 0 - high value and check how much wood you get on every mid value 
// and change the low and high according to that value.
        while(low<=high)
        {
            long long mid = high+(high-low)>>1;
                long long wood = 0 ; 
