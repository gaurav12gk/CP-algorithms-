...
int lpf[Max];//lpf= lowese prime factor of any number  
//Function to assign the lowest prime factor to every number from 1 - n 
 void  factorsieve(int n)
  {
        for(int i = 2;i<=n;i++)lpf[i]= i; 
        lpf[1]=lpf[0]=-1; 
        //Assinging all even number lowest prime factor as 2 ; 
        for(int i = 4;i<=n;i+=2)
        {
            lpf[i]=2;
        }

        //Assinging other numbers; 
        for(int i = 3; i*i<=n;i++)
        {
            if(lpf[i]==i)
            {
  ...
  // Function to get the factors of the number  
  void getfactors(int x)
  {
      ...
      cout<<" Factors of the number x is ==> ";
for(auto c: ans)cout<<c<< " ";debug(ans)
cout<<line;
  } 
